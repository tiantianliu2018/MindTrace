...
MOCK_ANALYSIS = {
    "emotion": "anxious",
    "emotions": [
        {"emotion": "anxious", "proportion": 0.6},
        {"emotion": "hopeful", "proportion": 0.4},
    ],
    "intensity": 0.65,
    "themes": ["工作", "压力"],
...
MOCK_SUMMARY = {
    ...
    "suggestion": "周二和周四情绪最低，留意这两天的触发模式。",
}


class TestCreateDiary:
    def test_create_diary_success(self, client, db_session):
        with patch("app.service.diary_service.analyze_text", return_value=MOCK_ANALYSIS):
        ...
        assert data["emotions"][0]["emotion"] == "anxious"
        assert data["emotions"][0]["proportion"] == 0.6
        assert data["trigger"] == "调试阻塞"
        assert data["physical_state"] == "疲劳"
        assert data["compared_to_previous"] == "比昨天更紧张一些。"

    def test_create_diary_empty_content(self, client):
        response = client.post("/api/diary", json={"date": "2026-05-16", "content": ""})
        assert response.status_code == 422

    def test_create_diary_future_date(self, client):
        response = client.post("/api/diary", json={"date": "2099-01-01", "content": "测试未来日期。"})
        assert response.status_code == 422

    def test_create_diary_content_too_long(self, client):
        response = client.post("/api/diary", json={"date": "2026-05-16", "content": "长" * 5001})
        assert response.status_code == 422
...
class TestGetSummary:
    ...
    def test_summary_period_week(self, client):
        with (
            patch("app.service.diary_service.analyze_text", return_value=MOCK_ANALYSIS),
