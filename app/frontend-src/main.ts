declare class Chart<TType extends string = string> {
...
const analysisEmotions = document.querySelector<HTMLElement>("#analysis-emotions");
const analysisTriggerBlock = document.querySelector<HTMLElement>("#analysis-trigger-block");
const analysisTrigger = document.querySelector<HTMLElement>("#analysis-trigger");
const analysisPhysicalBlock = document.querySelector<HTMLElement>("#analysis-physical-block");
const analysisPhysicalState = document.querySelector<HTMLElement>("#analysis-physical-state");
const analysisCompareBlock = document.querySelector<HTMLElement>("#analysis-compare-block");
const analysisCompared = document.querySelector<HTMLElement>("#analysis-compared");
...
function renderHistory(items: AnalysisResponse[]) {
  ...
  items.forEach((item) => {
    ...
    if (item.emotions && item.emotions.length > 1) {
      subEmotions = `<div class="history-sub-emotions">${
        item.emotions
...
const chartDays = document.querySelector<HTMLSelectElement>("#chart-days");
const chartEmpty = document.querySelector<HTMLElement>("#chart-empty");
const chartArea = document.querySelector<HTMLElement>("#chart-area");
let intensityChart: Chart<"line"> | null = null;
let distributionChart: Chart<"doughnut"> | null = null;
...
async function loadCharts() {
  ...
  const data = await requestJson<ChartData>(`/api/diary/chart?days=${days}`);
  ...
  const ctx1 = (document.querySelector("#intensity-chart") as HTMLCanvasElement).getContext("2d")!;
  ...
  const ctx2 = (document.querySelector("#distribution-chart") as HTMLCanvasElement).getContext("2d")!;
...
const calendarMonth = document.querySelector<HTMLInputElement>("#calendar-month");
const calendarArea = document.querySelector<HTMLElement>("#calendar-area");

const EMOTION_GROUP_COLORS: Record<string, string> = {
  positive: "#e7bb6f",
  neutral: "#7eb8b0",
  negative: "#c4683a",
  mixed: "#b8a08a",
...
function getEmotionGroup(emotion: string): string {
  if (!emotion || emotion === "unknown") return "unknown";
...
async function loadCalendar() {
  ...
  const data = await requestJson<CalendarDay[]>(`/api/diary/calendar?month=${month}`);
